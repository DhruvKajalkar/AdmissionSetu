  ...
  return (
    <>
      ...
      <div className="simulation-action-grid">
        <section className="simulation-action-card"><p>Action A</p><h2>Withdraw current seat</h2><p>Release Aarya&apos;s participating AISSMS seat and remove her current CAP admission from the demo.</p><button type="button" disabled={admission?.kind !== "PARTICIPATING_SEAT"} onClick={() => setPendingAction("WITHDRAW")}>Withdraw in Demo</button></section>
      ...
      {pendingAction ? (
        <section className="simulation-confirmation" role="alertdialog" aria-modal="false" aria-labelledby="simulation-confirm-title">
          <div><p>Confirm demo action</p><h2 id="simulation-confirm-title">{pendingAction === "WITHDRAW" ? "Release the AISSMS seat?" : pendingAction === "CONNECTED" ? "Confirm the connected demo admission?" : "Reset the admission simulation?"}</h2><p>{pendingAction === "WITHDRAW" ? "Aarya will no longer hold the AISSMS admission. AISSMS Computer Engineering demo vacancies will increase immediately." : pendingAction === "CONNECTED" ? "The fictional connected admission will become current. Aarya's AISSMS seat will be released and the vacancy board will update." : "The original AISSMS seat, synthetic inventory, and original event history will be restored. Phase 3 preferences will not be changed."}</p></div><div><button className="secondary-action-button" type="button" onClick={() => setPendingAction(null)}>Cancel</button><button className={pendingAction === "RESET" ? "danger-action-button" : "primary-action-button"} type="button" onClick={() => complete(pendingAction)}>{pendingAction === "RESET" ? "Reset Demo" : "Confirm Demo Action"}</button></div>
        </section>
      ) : null}
