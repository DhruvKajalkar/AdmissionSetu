...
export function VacancyExchange({ institutes, programs }: { institutes: readonly OfficialInstitute[]; programs: readonly OfficialProgram[] }) {
  ...
  const feedbackAvailableNow = state.lastFeedback
    ? getProgrammeVacancies(state, state.lastFeedback.programId)
    : null;
  ...
  return (
    <>
      ...
      <div className="vacancy-disclaimer"><strong>These are not official current CET vacancy numbers.</strong><span>The deliberately small inventory is synthetic. Institute and programme names come from the official CET reference catalog.</span></div>
      {state.lastFeedback && feedbackProgram && feedbackInstitute ? <div className="vacancy-change-banner" role="status"><span>Updated</span><div><strong>{feedbackInstitute.commonName} · {feedbackProgram.name}</strong><p>A released synthetic seat changed availability from {state.lastFeedback.availabilityBefore} to {state.lastFeedback.availabilityAfter}.{feedbackAvailableNow !== state.lastFeedback.availabilityAfter ? ` The released seat then moved to the next eligible candidate as an offer, so ${feedbackAvailableNow} remain available now.` : ""}</p></div></div> : null}
      <section className="vacancy-overview" aria-label="Demo vacancy totals"><div><span>Available now</span><strong>{totalAvailable}</strong><small>live demo seats</small></div><div><span>Represented programmes</span><strong>{rows.length}</strong><small>small synthetic inventory</small></div><p>Every count below is calculated directly from synthetic seats currently marked available.</p></section>
      ...
      <div className="vacancy-results-heading"><div><h2>{filtered.length} programme{filtered.length === 1 ? "" : "s"}</h2><p>Updated from the live demo seat state on this device.</p></div></div>
      <div className="vacancy-card-grid">{filtered.map(({ program, institute, available: count, inventory }) => <article className={count ? "vacancy-card available" : "vacancy-card full"} key={program.choiceCode}><div className="vacancy-card-top"><span>{institute.commonName}</span><em>{count ? "Available" : "Currently full"}</em></div><h3>{program.name}</h3><p>{institute.name}</p><dl><div><dt>Demo seats available</dt><dd>{count}</dd></div><div><dt>Demo seats represented</dt><dd>{inventory}</dd></div><div><dt>Choice code</dt><dd>{program.choiceCode}</dd></div></dl><div className="vacancy-card-actions"><Link href="/explore">View programme</Link><Link href="/spot-rounds">Explore spot rounds</Link></div></article>)}</div>
